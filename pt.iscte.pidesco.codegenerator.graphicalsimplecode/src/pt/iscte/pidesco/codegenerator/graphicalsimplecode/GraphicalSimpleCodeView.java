		        	...
		        	case SWT.Selection:
		        		if(combo.getSelectionIndex()==-1)
		        			...
		        			for(int i=0; i<children.length/2; i++) {
		        				System.out.println(((Label)children[i*2]).getText() + " " + ((Text)children[i*2+1]).getText());
		        				output=output.replaceAll("@"+((Label)children[i*2]).getText()+"@", ((Text)children[i*2+1]).getText());
		        			}
